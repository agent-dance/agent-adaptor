...
// fakeV1Driver is a programmable driver.Driver for the v1 handler tests.
type fakeV1Driver struct {
	mu       sync.Mutex
...
func (f *fakeV1Driver) request(t *testing.T, i int) driver.Request {
	t.Helper()
	f.mu.Lock()
	defer f.mu.Unlock()
	if i >= len(f.requests) {
...
func TestHandlerV1RawHappyPath(t *testing.T) {
	t.Parallel()
	fake := &fakeV1Driver{
		payloads: []driver.StreamPayload{
	...
	srv := httptest.NewServer(sse.HandlerV1(adaptor.New(fake), sse.OptionsV1{Protocol: sse.Raw}))
	defer srv.Close()
...
func TestHandlerV1AGUIHappyPath(t *testing.T) {
	t.Parallel()
	fake := &fakeV1Driver{
		payloads: []driver.StreamPayload{
			{Kind: driver.StreamRunStarted, ThreadID: "t", RunID: "r"},
	...
	// The core message lifecycle must appear in the legacy order; extra
	// CUSTOM notices from the v1 engine may interleave.
...
// v1Recorder is a channel-instrumented ResponseWriter: it signals the first
...
func newV1Recorder() *v1Recorder {
	return &v1Recorder{header: http.Header{}, wrote: make(chan struct{})}
}

func (r *v1Recorder) Header() http.Header { return r.header }
func (r *v1Recorder) WriteHeader(int)     {}
func (r *v1Recorder) Flush()              {}

func (r *v1Recorder) Write(p []byte) (int, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	n, err := r.buf.Write(p)
...
func (r *v1Recorder) String() string {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.buf.String()
}

// TestHandlerV1DisconnectCancelsRun is the mandated disconnect-cancel anchor,
// aligned item-by-item with the legacy handler semantics:
//
...
func TestHandlerV1DisconnectCancelsRun(t *testing.T) {
	t.Parallel()
	...
	handler := sse.HandlerV1(adaptor.New(fake), sse.OptionsV1{Protocol: sse.Raw})
	rec := newV1Recorder()
...
func TestHandlerV1RejectsEmptyPrompt(t *testing.T) {
	t.Parallel()
	handler := sse.HandlerV1(adaptor.New(&fakeV1Driver{response: driver.Response{Output: "ok"}}), sse.OptionsV1{Protocol: sse.Raw})
...
func TestHandlerV1RejectsWrongMethod(t *testing.T) {
	t.Parallel()
	handler := sse.HandlerV1(adaptor.New(&fakeV1Driver{response: driver.Response{Output: "ok"}}), sse.OptionsV1{Protocol: sse.AGUI})
	rec := httptest.NewRecorder()
