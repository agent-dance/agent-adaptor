...
func TestApprovalErrorHTTPMapping(t *testing.T) {
	tests := []struct {
		name   string
		err    error
		status int
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			sse.WriteApprovalError(recorder, test.err)
			...
			var response sse.ApprovalErrorResponse
...
func TestWriteApprovalErrorNilIsNoContent(t *testing.T) {
	recorder := httptest.NewRecorder()
	sse.WriteApprovalError(recorder, nil)
