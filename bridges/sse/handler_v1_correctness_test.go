...
func TestDecodeRequestV1PreservesRawThreadKeyAndSeparatesAGUITuple(t *testing.T) {
	rawKey := "tenant/a/b:c\x00d"
	rawReq := httptest.NewRequest("GET", "/?prompt=hello&sessionKey="+url.QueryEscape(rawKey), nil)
	_, gotRaw, err := decodeRequestV1(rawReq, OptionsV1{Protocol: Raw})
	if err != nil {
		t.Fatal(err)
	}
	if gotRaw != rawKey {
	...
	_, gotAGUI, err := decodeRequestV1(aguiReq, OptionsV1{Protocol: AGUI})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStreamRawV1UsesEventSequenceAndResultAsTerminalAuthority(t *testing.T) {
	...
	if err := streamRawV1(context.Background(), &sync.Mutex{}, recorder, stream, 0, false); err != nil {
		t.Fatal(err)
	}
...
func TestStreamRawV1SeedsMetadataFreeCursorFromLastEventID(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	if err := streamRawV1(context.Background(), &sync.Mutex{}, recorder, stream, 80, true); err != nil {
		t.Fatal(err)
	}
	body := recorder.Body.String()
...
func TestRawFrameV1PreservesApprovalDropAndEnvelope(t *testing.T) {
	...
	name, raw := rawFrameForV1(approval)
...
func TestRawTerminalInfrastructureFailureIsStructured(t *testing.T) {
	name, body := rawTerminalFrameV1("run", nil, nil, errors.New("transport broke"))
