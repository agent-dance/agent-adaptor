...
// Redaction anchor: the server keeps the conservative default ExposurePolicy —
...
func TestNewServerDefaultExposureOmitsDiagnostics(t *testing.T) {
	t.Parallel()
	...
	srv := a2a.NewServer(adaptor.New(fake), a2a.ServerOptions{AgentCard: testCard()})

	envelope := decodeTask(t, postRPC(t, srv.Handler(),
...
func TestThreadByContextIDRequiresAgentRunner(t *testing.T) {
	...
	srv := a2a.NewServer(agent.Thread("base-thread"), a2a.ServerOptions{
		AgentCard: testCard(),
...
func TestThreadByContextIDMintedContextStillThreads(t *testing.T) {
	...
	srv := a2a.NewServer(adaptor.New(fake, adaptor.WithThreadStore(memory.NewStore())), a2a.ServerOptions{
		AgentCard: testCard(),
		Session:   a2a.ThreadByContextID(),
	})
	...
	if envelope.Result.Task == nil || envelope.Result.Task.Status.State != "TASK_STATE_COMPLETED" {
		t.Fatalf("task = %+v (status: %s)", envelope.Result.Task, statusText(envelope))
	}
