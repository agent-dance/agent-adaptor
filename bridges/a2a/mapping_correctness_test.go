package a2a

import (
	"context"
	"encoding/json"
	"reflect"
	"strings"
	"testing"
	"time"

	a2aproto "github.com/a2aproject/a2a-go/v2/a2a"
	adaptor "github.com/agent-dance/agent-adaptor"
	"github.com/agent-dance/agent-adaptor/driver"
)

func dataValue(t *testing.T, event a2aproto.Event) any {
	t.Helper()
	update, ok := event.(*a2aproto.TaskStatusUpdateEvent)
	if !ok || update.Status.Message == nil || len(update.Status.Message.Parts) != 1 {
		t.Fatalf("status event = %#v", event)
	}
	data, ok := update.Status.Message.Parts[0].Content.(a2aproto.Data)
	if !ok {
		t.Fatalf("part = %#v", update.Status.Message.Parts[0])
	}
	return data.Value
}

func TestTranslatorAndDecoderPreserveEventMetaAndToolSnapshots(t *testing.T) {
	sourceTime := time.Date(2026, 7, 28, 1, 2, 3, 4, time.UTC)
	meta := adaptor.EventMeta{
		RunID: "run", ThreadKey: "opaque/thread:key", TurnID: "turn", Sequence: 17, Time: sourceTime,
		Source: &adaptor.EventSourceMeta{RunID: "provider-run", ThreadID: "provider-thread", TurnID: "provider-turn", Sequence: 99, Timestamp: sourceTime.Add(time.Second)},
	}
	input := adaptor.WithEventMeta(adaptor.ToolCall{
		ID: "tool", Name: "shell", Args: map[string]any{"cmd": "go test"},
		Result: map[string]any{"exit": 0}, Phase: adaptor.PhaseEnd,
	}, meta)
	translator := newStreamTranslator(testTaskInfo{}, ExposurePolicy{
		IncludeToolCalls: true, Diagnostics: DiagnosticsPolicy{IncludeMetadata: true},
	})
	events := translator.Translate(input)
	if len(events) != 1 {
		t.Fatalf("events = %#v", events)
	}
	decoded, matched, err := DecodeAdapterEventV1(dataValue(t, events[0]))
	if err != nil || !matched {
		t.Fatalf("decode matched=%v err=%v", matched, err)
	}
	tool, ok := decoded.(adaptor.ToolCall)
	if !ok || tool.Phase != adaptor.PhaseEnd || !reflect.DeepEqual(tool.Args, map[string]any{"cmd": "go test"}) || !reflect.DeepEqual(tool.Result, map[string]any{"exit": float64(0)}) {
		t.Fatalf("decoded tool = %#v", decoded)
	}
	gotMeta := decoded.Meta()
	if gotMeta.RunID != meta.RunID || gotMeta.ThreadKey != meta.ThreadKey || gotMeta.Sequence != 17 || !gotMeta.Time.Equal(meta.Time) || gotMeta.Source == nil || gotMeta.Source.ThreadID != "provider-thread" {
		t.Fatalf("decoded meta = %#v", gotMeta)
	}
}

func TestApprovalAndDroppedRoundTripAreCompleteAndPolicyGated(t *testing.T) {
	deadline := time.Date(2026, 7, 28, 2, 3, 4, 0, time.UTC)
	approval := adaptor.WithEventMeta(&adaptor.ApprovalRequest{
		ID: "req", Kind: adaptor.ApprovalQuestion, Source: "provider", Title: "choose",
		Details: map[string]any{"path": "safe"}, Choices: []adaptor.Choice{{Key: "a", Label: "A", Description: "first"}},
		ToolCallID: "tool", Deadline: deadline, Attempt: 3,
	}, adaptor.EventMeta{RunID: "run", Sequence: 1})

	hidden := newStreamTranslator(testTaskInfo{}, ExposurePolicy{})
	if got := hidden.Translate(approval); len(got) != 0 {
		t.Fatalf("default policy exposed HITL: %#v", got)
	}
	exposed := newStreamTranslator(testTaskInfo{}, ExposurePolicy{
		IncludeHITL: true, Diagnostics: DiagnosticsPolicy{IncludeHITLPayloads: true},
	})
	events := exposed.Translate(approval)
	decoded, matched, err := DecodeAdapterEventV1(dataValue(t, events[0]))
	if err != nil || !matched {
		t.Fatalf("decode matched=%v err=%v", matched, err)
	}
	request := decoded.(*adaptor.ApprovalRequest)
	if request.ID != "req" || request.Kind != adaptor.ApprovalQuestion || request.Title != "choose" || request.ToolCallID != "tool" || request.Attempt != 3 || !request.Deadline.Equal(deadline) || len(request.Choices) != 1 || request.Choices[0].Description != "first" {
		t.Fatalf("approval = %#v", request)
	}

	dropped := adaptor.WithEventMeta(adaptor.Dropped{
		Count: 4, ByKind: map[string]int{"text.content": 3, "thinking.content": 1},
		FirstSequence: 2, LastSequence: 5, Reason: "backpressure", Source: "sdk", Details: map[string]any{"capacity": 1},
	}, adaptor.EventMeta{RunID: "run", Sequence: 6})
	if got := hidden.Translate(dropped); len(got) != 0 {
		t.Fatalf("default policy exposed drop diagnostics: %#v", got)
	}
	diagnostic := newStreamTranslator(testTaskInfo{}, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeUsage: true}})
	events = diagnostic.Translate(dropped)
	decoded, matched, err = DecodeAdapterEventV1(dataValue(t, events[0]))
	if err != nil || !matched {
		t.Fatalf("decode drop matched=%v err=%v", matched, err)
	}
	drop := decoded.(adaptor.Dropped)
	if drop.Count != 4 || drop.FirstSequence != 2 || drop.LastSequence != 5 || drop.ByKind["text.content"] != 3 || drop.Reason != "backpressure" || drop.Details["capacity"] != float64(1) {
		t.Fatalf("drop = %#v", drop)
	}
}

func TestHITLNoticeRawDiagnosticsAreOptInAndSanitized(t *testing.T) {
	notice := adaptor.Notice{
		Kind: adaptor.NoticeApprovalResolved,
		Data: map[string]any{
			"request_id":    "req-1",
			"kind":          "permission",
			"result":        "approved",
			"detail":        "visible",
			"authorization": "Bearer top-secret",
		},
	}

	hidden := hitlNoticeArtifact("hitl.resolved", notice, ExposurePolicy{IncludeHITL: true})
	if _, ok := hidden["raw"]; ok {
		t.Fatalf("raw HITL diagnostics must be opt-in: %#v", hidden)
	}

	exposed := hitlNoticeArtifact("hitl.resolved", notice, ExposurePolicy{
		IncludeHITL: true,
		Diagnostics: DiagnosticsPolicy{IncludeHITLRaw: true},
	})
	raw, ok := exposed["raw"].(map[string]any)
	if !ok || raw["detail"] != "visible" {
		t.Fatalf("raw HITL diagnostics = %#v", exposed["raw"])
	}
	serialized, err := json.Marshal(raw)
	if err != nil {
		t.Fatalf("marshal raw HITL diagnostics: %v", err)
	}
	if strings.Contains(string(serialized), "top-secret") {
		t.Fatalf("raw HITL diagnostics leaked a secret: %s", serialized)
	}
}

func TestRunFinishedNeverCreatesASecondA2ATerminal(t *testing.T) {
	translator := newStreamTranslator(testTaskInfo{}, ExposurePolicy{})
	if got := translator.Translate(adaptor.RunFinished{RunID: "run", Failed: true, Reason: adaptor.ReasonAgentError, Message: "misleading"}); len(got) != 0 {
		t.Fatalf("informational RunFinished emitted terminal status: %#v", got)
	}
}

func TestBusinessFailureDetailsRespectExposurePolicy(t *testing.T) {
	runError := &adaptor.RunError{
		Reason: adaptor.ReasonPolicyViolation, Message: "blocked",
		Details: map[string]any{"rule": "no-write", "authorization": "Bearer secret"},
	}
	hidden := failureDetails(runError, ExposurePolicy{})
	if !reflect.DeepEqual(hidden, map[string]any{"code": string(adaptor.ReasonPolicyViolation)}) {
		t.Fatalf("default failure details = %#v", hidden)
	}
	exposed := failureDetails(runError, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeMetadata: true}})
	metadata, ok := exposed["metadata"].(map[string]any)
	if !ok || metadata["rule"] != "no-write" {
		t.Fatalf("exposed failure details = %#v", exposed)
	}
	serialized, _ := json.Marshal(metadata)
	if strings.Contains(string(serialized), "secret") {
		t.Fatalf("failure metadata leaked secret: %s", serialized)
	}
}

func TestProviderTerminalExposureIsExplicitSanitizedAndValidJSONOnly(t *testing.T) {
	result := resultWithRawForTest(t, adaptor.RawStreams{Terminal: &driver.TerminalPayload{
		Event: "turn/completed", JSON: json.RawMessage(`{"authorization":"Bearer secret","answer":42}`),
	}})

	hidden := defaultTerminalArtifacts(testTaskInfo{}, result, ExposurePolicy{})
	if artifactData(t, hidden)["provider_result"] != nil {
		t.Fatalf("default exposure leaked provider terminal: %#v", artifactData(t, hidden))
	}
	exposed := defaultTerminalArtifacts(testTaskInfo{}, result, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeProviderResult: true}})
	provider, ok := artifactData(t, exposed)["provider_result"].(map[string]any)
	if !ok || provider["event"] != "turn/completed" {
		t.Fatalf("provider result = %#v", provider)
	}
	serialized, _ := json.Marshal(provider)
	if string(serialized) == "" || strings.Contains(string(serialized), "secret") {
		t.Fatalf("provider terminal was not sanitized: %s", serialized)
	}

	result = resultWithRawForTest(t, adaptor.RawStreams{Terminal: &driver.TerminalPayload{Event: "bad", JSON: json.RawMessage(`{`)}})
	invalid := defaultTerminalArtifacts(testTaskInfo{}, result, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeProviderResult: true}})
	if artifactData(t, invalid)["provider_result"] != nil {
		t.Fatalf("invalid JSON terminal must be omitted: %#v", artifactData(t, invalid))
	}
}

func TestUsageExposureDistinguishesUnobservedFromObservedZero(t *testing.T) {
	unobserved := resultWithUsageForTest(t, nil)
	events := defaultTerminalArtifacts(testTaskInfo{}, unobserved, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeUsage: true}})
	if _, ok := artifactData(t, events)["usage"]; ok {
		t.Fatalf("unobserved usage must be omitted: %#v", artifactData(t, events))
	}

	observedZero := resultWithUsageForTest(t, &driver.Usage{})
	events = defaultTerminalArtifacts(testTaskInfo{}, observedZero, ExposurePolicy{Diagnostics: DiagnosticsPolicy{IncludeUsage: true}})
	if _, ok := artifactData(t, events)["usage"]; !ok {
		t.Fatalf("observed zero usage must remain observable: %#v", artifactData(t, events))
	}
}

type rawResultDriver struct{ raw adaptor.RawStreams }

func (d rawResultDriver) Descriptor() driver.Descriptor { return driver.Descriptor{Type: "raw-test"} }
func (d rawResultDriver) ValidateConfig(any) error      { return nil }
func (d rawResultDriver) Run(context.Context, driver.Request, driver.EventSink) (driver.Response, error) {
	return driver.Response{Summary: "summary", RawStreams: &d.raw}, nil
}

func resultWithRawForTest(t *testing.T, raw adaptor.RawStreams) *adaptor.Result {
	t.Helper()
	result, err := adaptor.New(rawResultDriver{raw: raw}).Run(context.Background(), "prompt")
	if err != nil {
		t.Fatal(err)
	}
	return result
}

type usageResultDriver struct{ usage *driver.Usage }

func (d usageResultDriver) Descriptor() driver.Descriptor {
	return driver.Descriptor{Type: "usage-test"}
}
func (d usageResultDriver) ValidateConfig(any) error { return nil }
func (d usageResultDriver) Run(context.Context, driver.Request, driver.EventSink) (driver.Response, error) {
	return driver.Response{Summary: "summary", Usage: d.usage}, nil
}

func resultWithUsageForTest(t *testing.T, usage *driver.Usage) *adaptor.Result {
	t.Helper()
	result, err := adaptor.New(usageResultDriver{usage: usage}).Run(context.Background(), "prompt")
	if err != nil {
		t.Fatal(err)
	}
	return result
}

func artifactData(t *testing.T, events []a2aproto.Event) map[string]any {
	t.Helper()
	artifact := events[0].(*a2aproto.TaskArtifactUpdateEvent)
	return artifact.Artifact.Parts[0].Content.(a2aproto.Data).Value.(map[string]any)
}
