...
func observationMeta(meta adaptor.EventMeta) map[string]any {
	out := map[string]any{"run_id": meta.RunID, "sequence": meta.Sequence, "time": meta.Time}
	if meta.ThreadKey != "" {
		out["thread_key"] = meta.ThreadKey
	}
	if meta.TurnID != "" {
...
func toolCardID(runID, scopeID, id string) string {
	var b bytes.Buffer
	encoder := json.NewEncoder(&b)
	encoder.SetEscapeHTML(false)
	// String arrays always encode; the trailing newline is not part of the tuple.
	_ = encoder.Encode([]string{"tool", runID, scopeID, id})
	return "aa1:" + base64.RawURLEncoding.EncodeToString(bytes.TrimSuffix(b.Bytes(), []byte{'\n'}))
}
func toolParent(e adaptor.ToolCall) aguievents.Event {
	return customEvent("adapter.tool.parent", map[string]any{"meta": observationMeta(e.Meta()), "tool_call_id": e.ID, "scope_id": e.ScopeID, "parent_scope_id": e.ParentScopeID, "parent_tool_call_id": e.ParentToolCallID})
}
