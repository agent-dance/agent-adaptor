...
> 状态：已落地记录 + repeatable smoke evidence + remaining gaps。本文承接 [`workstream-effective-profile-materialization-plan.md`](./workstream-effective-profile-materialization-plan.md) 已拍板的 profile 管理规范，参考 skills / MCP 已落地的执行机制，记录 `agents`、`hooks`、`instructions`、`config` 四类 profile resource 的当前实现状态。
>
> Review verdict：公共能力不按“最小交集”设计，而按“最大可努力支持的 capability envelope”设计：core 字段表达三家都能理解的 host intent；extended 字段表达至少一家有稳定 native surface、其它家有明确 fallback/unsupported 的能力；provider-native 细节通过显式 escape hatch 暴露。`AgentSpec` / `HookSpec` / `InstructionsBundleRef` / `ProfileConfigPatch` 的公共语义已经在当前实现里有对应入口，本文继续收敛的是字段级 capability 和 materialization 语义，而不是资源种类是否存在。
...
- 为 unsupported/fallback behavior 增加/保留测试，确保尚未覆盖的字段级能力不会被报告为 native managed。

验收：

- `SyncProfile` 对已支持的 skills / MCP 报 managed。
- `SyncProfile` 对已实现的 agents / hooks / instructions / native config patch 报告真实 materialization；对仍在扩展中的 config capability patch 和 unsupported extended 字段报 warning/error，不能假装 managed。
...
- skills / MCP 已接入 profile-local manifest / lock / ownership 路径；MCP synced snapshot 会报告真实 managed / external keys，skills manifest 记录 hashed source metadata，避免泄漏 source path 或 skill content。
- Remaining risk：hooks 当前采用保守 ownership，细粒度 settings merge/takeover/diff/backup 仍待第二期安全治理。
...
- 不做 hook safety governance（approval UI、diff/rollback/backup/audit）；这部分只保留为 remaining risk。
