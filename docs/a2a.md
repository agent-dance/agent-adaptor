...
`MaxArtifacts` still limits only compact final Artifacts, including explicit zero. Live updates and opt-in full RemoteArtifacts remain complete. A truncation emits a safe `DelegationStreamDropped` with `reason=artifact_result_limit`, `omitted_count`, and `max_artifacts`. The same compact limit applies to partial results when polling ends through cancellation, a deadline or the active execution budget. It does not remove full opt-in remote parts or replace the original failure cause.
