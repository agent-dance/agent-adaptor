...
| [Alignment task dispatch package](./alignment-tasks/2026-09-07/README.md) | Seven gated batches with 47 task.json files, independent ownership, 96 traceable requirements, and dispatch validation. |

2026-09-07 internal 对齐的生产改动已通过 B04；B05 的独立跨层验证、Driver 一致性、CI、文档示例与 R021 常驻回调修复、R022 接收同步验证及 R023 stdin/RPC 关闭修复由 G05 在合流提交上验收。原生 Linux 曾发现 CodeBuddy 常驻轮次交接 race，旧 G05 证据保留，所有 B06 验收必须使用修复后的新冻结提交。冻结规范见 [合同清单](alignment-tasks/2026-09-07/contracts/frozen.json)，无需 CLI 的使用入口见 [offline 示例](../examples/offline/main.go)。T25–T30 必须使用 G05 冻结的同一提交补齐原生平台与真实 provider 证据；本页和 CI 配置不代替实际验收。
