...
| P3.2 | 🟡 词汇包已完成（提交 61b76ba）：`skill/` 包 `Dir`/`FS`/`Inline`/`Key`/`Require` + **`Archive(key, open, opts...)`**（P0.7 缺口补齐：Opener= 根包归档 helper 别名，`ArchiveBytes/File/URL`，`WithFormat`/`WithSubpath`/`WithFingerprint` 走不透明 config，P5 搬真身签名零变）+ `Ref`/`Skill`/`Source`/`Provider`/`Catalog`/`Set`/`Materializer` 别名族；15 用例含三格式×auto/显式共 6 路真实物化验证。**待办**：① `WithSkills`/`WithSkillProvider`/`WithSkillMaterializer` 接线留给 P3 接线波；② 收编 `archive_*.go` 真身 + `LocalSkillsFromDir`/`SkillsAsRefs`/`NewDefaultSkillMaterializer` 族留给 P5；③ **发现盲区**：SkillFromArchive.Fingerprint godoc 称作缓存键但默认物化器实际忽略（纯内容寻址，engine/skill_helpers.go writeFromArchive）——收编时二选一收敛；④ skill→根包依赖 P5 翻转（WithSkills 用 driver.SkillRef 形状即无环） | `skill_*.go` 5 个文件 + `archive_*.go` + `internal/skillruntime` |
...
| P3.4 | 🟡 词汇包已完成（提交 0b670ed）：`profile/` 包 `Selection = driver.ProfileSelection` + `Default`/`Native`/`Dedicated`/`CloneNative`/`CloneFrom` 构造器（`Default()` 为现状 Unset 形态的显式 v1 名）+ CloneOption 族（`CopySettings`/`CopyMCP`/`CopySkills`/`CopyAuth`/`LinkAuth`/`WithOptions` 逃生舱，遗留 IncludeAuth 经 WithOptions 无损迁移）+ `Resources = ProfileResources` 及 SubAgent/Hook(20 事件全量)/Instructions(`Text()` 构造)/ConfigPatch 别名族，22 用例对照旧 root 选项 DeepEqual 全绿。**待办**：`WithProfile`/`WithProfileResources` 选项接线留给 P3 接线波；S8 的 `SubAgents` 字段名需 P5 搬真身时定夺（本波以 `Agents:` 承接） | `profile.go` `profile_resources.go` `internal/profile*` 8 个包 |
