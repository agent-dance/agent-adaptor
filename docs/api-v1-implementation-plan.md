...
| 阶段 | 最终状态 | 证据/结果 |
|---|---|---|
| P5.2 PRE/CORRECTNESS/REHOME | **已关闭** | `46a726f` 完成 PRE 依赖反转；`82d2f00` 完成执行、Thread、Driver 协议、P4/SPI/bridge 正确性基础；公共类型不再泄漏 `internal/*` |
| LEGACY EDGE DELETE | **已关闭** | 旧 bridge/hosttool/memory/provider sugar、旧 adaptertest、`pkg/` forwards 与旧根消费者在 cutover 前清除 |
| ROOT CUTOVER | **已关闭** | `cc6cd82`：根 import path 原子切换为 package `adaptor`，旧中央 SDK/registry/RunHandle API 与 staging `next/` 同时退出 |
| RESIDUAL DELETE | **已关闭** | 删除 `internal/engine` 中旧 Core/binding/admin/execute/decision/options 死栈、旧 metadata fallback、迁移 aliases/wrappers 与无调用代码；保留 v1 Thread/Inspect/profile/skill/runtime 自由函数 |
...
| P0.2 | `internal/engine` 抽取：run 管线、选项合并、会话协调、checkpoint。**波及面修正（P0.7 盘点）**：engine 不能回 import 根包，管线引用的全部合同类型必须随迁并别名回指，实际波及 ≈ 全部 32 个非测试文件（逐文件路线图见 [p0-inventory.md](./archive/p0-inventory.md)）；`archive_*.go` 是 skill 归档源，**不在本任务**（归 P3.2） | 根包全量（见归档 inventory 映射表） |
...
| P0.7 | ✅ 已完成：[p0-inventory.md](./archive/p0-inventory.md)（54 文件逐一映射 + 四裁决 D9/D10/D11 + `run_policy.go` 四分方案）。注意：`EffectiveHumanDecisionPolicy` 被 claude/codebuddy 4 处调用，须经 driver 包保留等价物 | 清单已产出 |
