...
按 R011，Observer/demand/publisher 的对象地址与纯观测需求不参与会话 fingerprint。Request.Streaming 是本轮 transport 选择；同一已配置 Driver 在同一 SessionCodec 下经正式协议证明可互相续接的 transport，不得仅因该布尔、临时 schema 机制或观测需求不同而改变 Thread 兼容 fingerprint、重绑记录或分配新的内部会话。Thread fingerprint 继续完整覆盖 Driver 构造配置及 codec/version、模型/identity、实际 workspace、真实 catalog/profile/Tools/skills/instructions/MCP/runtime 等影响续接的维度。transport 若实际改变 checkpoint 编码、会话环境或其它续接语义，必须在既有 SessionConfigFingerprinter / SessionCodec 参数及 Driver 启动前 guard 中稳定表达并拒绝不兼容；不能由 helper 猜测或一概忽略。实际进程启动参数和环境仍由 Driver 私有启动签名/eligibility 完整比较；临时 schema 轮先有界停旧 writer，成功后以同一健康 checkpoint 构建后续常驻 spec，临时轮的参数不得污染该 prewarm spec。WithSpawn 本轮不注册为 writer，所有路径保持单 writer 与交付后不重放。T06接入真实协商，T10仅扩展同一resolver，不另建pipeline。
