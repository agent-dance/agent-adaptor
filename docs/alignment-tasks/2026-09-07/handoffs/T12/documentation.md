...
## Attempt 3 S9 fixture migration (R015 / G03-F01)

The first G03 integration at `372a46588382e3d083d70a90f7de4546427cf185` exposed the remaining S9 caller of post-start `subagentstream.Merge`. Its `ErrEventInjectionUnsupported` failure was independently reproduced on the unchanged T12 candidate `3769ffde2bc65df43c198ee82d171e6770f304a8` before this migration. The original gate log and worker failure log are retained with the handoff evidence.

S9 now constructs the leader with `adaptor.New(fakeLeader, team.Option())` and reads `stream.Events()` and `stream.Result()` directly. Its fake leader actually calls the loopback HTTP MCP sidecar, so its Descriptor declares `MCPCapability{Supported: true, HTTP: true}`. The removed wrapper RunID equality becomes an original Stream RunID nonempty/stable assertion. Every existing assertion for the three members' ordering, typed updates, text deltas, result-before-terminal visibility, HasLine, Observe and Close remains in place. This is the same-batch consumer migration required by the already documented Merge breaking change; it adds no public declarations or production behavior and does not alter Merge's rejection rule or provide T18's future binding proof.

The final attempt-3 source SHA is checked with the original full four-package validation, four-package race and vet, plus the complete `hosttools/a2adelegation` package and S9 under `-race -count=5`. All provider/live gates remain zero. G03 must independently rerun the integration gate after accepting this fixture change; local S9 success is not whole-repository or release acceptance.
