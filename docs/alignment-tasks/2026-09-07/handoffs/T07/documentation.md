...
### Attempt 3 follow-up: absent versus observed zero Usage

The 7e1fb39 independent review confirmed the original three F02 repairs and retained F01 behavior, then found that empty, unknown, negative or fractional message counters allocated non-nil zero Usage. The four supplied overlay cases were reproduced before this follow-up; the prior 7e1fb39 result and complete check logs remain preserved separately.

Usage counters now use a local strict conversion: only nonnegative integers within the SDK int range count as observations. A usage object with no valid recognized counter allocates no Usage; explicit zero still does. Message start/delta, assistant snapshot and terminal usage use the same local counter rule. Terminal usage with a valid counter retains authority over aggregated messages; an entirely invalid terminal usage object leaves observed message totals available. General protocol integer conversion and all message-boundary behavior remain unchanged.

TestAlignmentClaudeUsageObservation covers absent/unknown/string/negative/fractional/out-of-range/null/bool counters, each recognized explicit-zero field, valid counters and mixed valid/invalid objects across the four formal frame sources. TestAlignmentClaudeTerminalUsageAuthority verifies invalid-terminal fallback and zero/positive terminal authority. G02 should include the absent-versus-observed-zero distinction in the same Usage paragraph, with no new public symbols or API golden change.
