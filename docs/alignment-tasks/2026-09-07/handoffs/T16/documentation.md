...
B06 已交付五个真实 `TestAlignmentLive*` 场景：NativeAppend（exec/app-server fresh/resume、app-server fork、无 append nonce 对照）、ObservationAndPersistent（typed skill/MCP/plan 与两轮复用）、SubagentCatalog（正式 child role + canonical key）、CancellationAndAppendRebind（取消部分输出、原健康 checkpoint、ResumeOnly 拒绝与空清除新建）、DedicatedToolResumeAfterClose（R017 跨 Agent 冷续）。既有 TestCodexDriverConformance live 分支与 appserver live 测试亦补齐 build tag + env 双门和独立 profile/workspace。两门开启后 CLI/证据缺失失败，不能 skip 必需场景。

R017 新用例固定同一真实 store、Dedicated source、workspace、四字段 Identity 与带稳定 Revision 的 WithTools catalog。Agent A 首轮实际调用工具并确认随机 128-bit nonce，公开 Checkpoint 必须健康；有界 Close 完成后，验证原生 `sessions/**/rollout-*-<session-id>.jsonl` 内容保全，再创建 Agent B 以同 key ResumeOnly。第二轮 prompt 不带 nonce，工具始终只回固定 ACK；响应必须精确回忆 nonce、真实调用新工具注册并保持同一 provider session ID。两轮各需一个 ProcessSpawn、正式 ProviderProtocol MCP Completed、Raw/Terminal/Transcript。测试只检查自有临时文件树中的原生 config.toml；两次 effective home 相同、credential carrier 不同。A 运行时匿名请求须被拒绝，Close 后成功绑定旧 loopback 地址证明旧 listener 已撤销，同时占住旧端口强制 B 轮换 endpoint。该 oracle 不抽取 bearer secret，也不声称检验了旧 token 在新 endpoint 上的响应。
