...
现在，常驻非 fallback 结果统一由同一个正式 parser 构建 Response，再原样返回 transport/context cause。`Thread.Run` 和 `Thread.Stream().Result()` 的失败返回仍为 `nil, error`；用 `errors.As` 取得 `*adaptor.RunError`，从其 `Result` 读取可用的 Text、Raw stdout/stderr/正式 Terminal、Transcript、Usage、Model、Provider、Metadata 与 Services。partial-message 中已观察到的 Usage 在尚无终局聚合用量时保留；正式终局用量仍优先，已观察零值不会变成 nil。未观察到的 runtime service 不产生成功报告。
...
失败进程退出时先排空 stdout，再等待 stderr copy 完成并 finalize parser，保留失败终局之后的原始诊断和没有换行的 stderr 尾部。断流仍保留常驻 transport 原本返回的 EOF；本改动不把它伪装成已观察到的具体 OS exit code 或 signal。
...
- `docs/public-errors.md` 的执行后部分 Result / CodeBuddy 边界：常驻错误 Response 不再为空，原 cause 可达，EOF 不伪造具体进程状态。
...
- `CHANGELOG.md` 的修复段落：CodeBuddy 常驻中断保留部分结果、stderr/失败终局 stdout 尾部和已观察用量；保留交付后不重放与 checkpoint 健康要求。
...
本地为 macOS arm64 / Go 1.26.5，使用本测试二进制的 CodeBuddy 协议子进程，未调用真实 provider。失败历史包括修复前空结果、缺少 partial Usage，以及失败终局后 Raw stdout 尾部丢失。测试覆盖 Run/Stream 逐字段等价、取消/deadline/非零/缺终局/畸形 JSON/畸形终局/正式 provider error、首次中断不落 checkpoint、原健康 record 不变、replacement 单 writer 和终局文本/Summary/零用量合同；既有包级测试继续覆盖 WithSpawn、重建、预热和安全 fallback。
