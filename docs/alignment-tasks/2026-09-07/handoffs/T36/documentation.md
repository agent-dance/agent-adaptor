...
Caller cancellation/deadline and custom causes remain visible with errors.Is; official RPC errors remain visible with errors.As. A private shutdown cause distinguishes Client.Close from a caller deliberately supplying jsonrpc2.ErrClosed as its cancellation cause. Client-only shutdown remains IsDisconnected without becoming caller cancellation. For an admitted operation, a shutdown cause already selected by Close is not relabelled by later caller cancellation. Before admission, an already-established caller error is preserved; otherwise closed admission returns ErrClosed. No exported declarations or API/golden changes are needed.
