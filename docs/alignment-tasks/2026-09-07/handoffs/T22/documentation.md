...
- R016 的公开 CancelTask ack 仍可先给无安全 code 的 canceled；它与 executor drain 的分类是不同可观察边界。真实 HTTP 用屏障核验实际第二次 Events、完整关闭、一次 Result、一次 Stream 和无伪造 limit。翻译错误保持协议失败，不被尾 hint 改为成功。内部 collector 分类引用固定 G04/T19 已验收证据，不计 T22 新独立执行数；Local 取消 drain 独立保留 partial/cause，并遵守本次 Delegate 已选主因。
