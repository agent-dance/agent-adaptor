...
- R016 的公开 CancelTask ack 仍可先给无安全 code 的 canceled；它与 executor drain 的分类是不同可观察边界。真实 HTTP 用屏障核验实际第二次 Events；Result 内同时要求 producer 已关闭且缓冲尾 channel 已清空，并写入测试直接检查的 earlyResult 标志。HTTP 在有界等待独立 resultDone 屏障后才检查一次 Result、一次 Stream 和无伪造 limit，不能将 ACK/EOF 或 producer close 当作消费完成。翻译错误按固定 SDK 的既有基础设施路径返回 failed Task+EOF，或公开 StreamRecoveryError 且非 nil Cause 逐字为 `encode adapter stream status: invalid_payload`；后者只能伴随无终态或已有 failed。手写 oracle 拒绝普通错误、超时、空 EOF、其他终态及任何 failure control envelope；正反控制覆盖该闭集，不把尾 hint 当成新分类或成功。内部 collector 分类引用固定 G04/T19 已验收证据，不计 T22 新独立执行数；Local 取消 drain 独立保留 partial/cause，并遵守本次 Delegate 已选主因。
...
| W13-R09 | 内层 R016HintQualification 16×HTTP/Local×Send/Stream、R016RealCoreSources 真实 pre-Driver 2×HTTP/Local×Send/Stream、CancellationAndTranslationDrain、DrainOracleConsumption 早读反向控制、TranslationOutcomeControls 严格失败形状正反控制 |
