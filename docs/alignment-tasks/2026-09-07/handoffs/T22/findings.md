...
## 新 G04 完整 race 验证中的翻译错误 oracle 校准

固定 `ce10dd3aef0d5c9b21f7c2c4871a6b49a1e13bc8` 的原 race count=20 完整执行后 exit=1：外层 2559 pass/1 fail，内层 1818 pass/2 fail（同一 translation=true 叶项及其父项），实际 binary 执行 20 次。原日志为 `evidence/T22-V02-ce10dd3-red.jsonl` 及 metadata，提取与计数为 `evidence/ce10dd3-translation-boundary-finding.json`。同 SHA 单场景额外 race100 未复现；该绿色不能覆盖原红。

F01 的尾消费/Result 完成/earlyResult/一次 Stream 屏障均通过，失败仅在原 oracle 要求 failed Task+EOF。一次实际返回“a2a stream disconnected before terminal state: encode adapter stream status: invalid_payload”，尚未观察 Task。协调者与原 C02 独立核对 pinned a2a-go/v2@v2.3.1 的 runProducerConsumer、processor.ProcessError 和 Client.startStream：processorResult 可转 failed Task；未形成该结果时 producer 原错误经公开 StreamRecoveryError 保留。两者均属于 C02 既有独立基础设施错误路径，不是生产缺陷或 R016 新决策。

按明确裁定，仅 translation=true 的独立 oracle 同时允许 failed+EOF，或 errors.As StreamRecoveryError 且非 nil Cause.Error() 逐字等于 `encode adapter stream status: invalid_payload`，状态仅允许无终态/已有 failed。任何 failure control envelope、其他终态、普通同文案 error、取消/deadline、nil Cause、错误来源、子串匹配、空/包装 EOF 均拒绝。新增两种正控（含已有 failed 的对照）与相应负控证明并非吞错。原 drain 屏障与公开 CancelTask ACK 断言不变；不修改生产/canonical。所有源及片段最后提交后重新执行完整原 V01/V02，旧 ce10dd3 结果仅为历史发现。
