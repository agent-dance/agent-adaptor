...
| `task_update_tool_tasksToTodos`, `TaskUpdateTool.execute` | todos 项 id/content/status 分别来自 task.id/subject/status。更新输入 taskId；成功 rawResponse 含 task/todos，删除成功也含完整 todos。缺任务/无更新/异常为工具错误，不应用请求。无正式 task/todos 时不因 rawResponse 是对象而确认；仅精确成功文案可确认输入。文案字段按 subject、description、activeForm、owner、metadata、blocks、blockedBy、status 的固定顺序，以逗号空格连接；blocks/blockedBy 分别对应 addBlocks/addBlockedBy。 |
...
Attempt 2 的 `alignment_independent_review_test.go` 保留独立审查的 Skill 别名/TaskUpdate 原期望并补反向无效 alias、未知 additive、空/未知 metadata 与精确文案控制；`alignment_partial_parent_review_test.go` 保留 root/foreign/malformed delta 对照及原始参数事件。原独立 fixture 字节及红日志在 evidence/attempt-2，后续全范围日志仅采自返修提交。
