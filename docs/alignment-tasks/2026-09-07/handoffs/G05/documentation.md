# G05 independent QA and documentation integration

All five workers use accepted replacement G04 b2035bc793369fb8fefb9de229ff1dbd2b748853,
canonical22 / C01–C04 / R001–R020. The following accepted source commits are
merged in manifest order; worker evidence remains external and immutable.

| Worker | Accepted source SHA | Final checks |
|---|---|---|
| T20 | `2d50f024c5c51d7c8c8d55dce11c92988748cf37` | T20-V01, T20-V02, T20-V03 |
| T21 | `5096368c1188b537f1f1c25d4c9ee2cd42cb0a26` | T21-V01, T21-V02 |
| T22 | `3dcd8626a1342ce7b548440db8bf2780a954589c` | T22-V01, T22-V02 |
| T23 | `1d92e2673e3f17ad674b3b074e565727a8befaf1` | T23-V01, T23-V02, T23-CONFORMANCE, T23-VET, T23-GATE-CLAUDE, T23-GATE-CODEBUDDY, T23-GATE-CODEX, T23-GATE-CURSOR, T23-WINDOWS-COMPILE, T23-EXAMPLE |
| T24 | `849f8334e376d4719167afa31706b4dea22b8cce` | T24-V01, T24-OFFLINE |

T20 contributes independent complete wire/audit literals, actual fake processes,
bounded lifecycle/cleanup barriers and R020 successor-ownership fault injection.
T21 retains its original protocol bytes and published-snapshot/race oracles;
the production defects they exposed are fixed by original owners T15/T32.
T22 independently tests policy timing and public composition; its drain oracle
checks consumption and a completed Result barrier, not merely producer closure.
T21/T22 strictly distinguish failed Task and typed infrastructure translation
outcomes, with negative controls; original red/stage logs remain historical.
T23 closes verifier/architecture blind spots and provides native/paid entrypoint
inventories. Only -json/-v additions and external process bounds supplement
the original required commands. T24 provides the actually executable offline
example and all five README/reference updates with a thirteen-work-item map.

Root reconciled each fragment into CHANGELOG, root godoc, AGENTS audit notes,
the documentation map and streaming/Driver/tool/policy/schema/A2A guides.
The final root/SPI public declarations, goldens, dependencies and generated
schema remain unchanged from accepted G04; there is no new runtime pipeline.
Actual default exposure, one error verdict, unchanged cancellation ACK and
separate evidence for resource materialization/input acceptance/execution remain.

This source fragment precedes the exact G05 checks. The final committed SHA
must pass the original validator/full Go suite/vet before acceptance and before
implementation_head is assigned. Worker-local passes are not aggregate passes.
T25–T30 must all verify that same frozen SHA without source commits. The
available Linux runner may execute only after G05 acceptance; missing native
Windows or paid authorization is recorded as missing, never converted to pass.
No push, tag, release, paid call or final release readiness is authorized here.

G05 attempt 1 at 9e12566c01b3626126ba85b035dcc2f5afe47126 failed the existing A2A import-localization guard. Original owners repaired T21/T22 QA imports and reran all original checks plus the unchanged guard. C02 confirmed that public StreamRecoveryError.Cause promises error semantics, not an upstream concrete type/code/details representation. Those extra coordinator assertions were removed; exact fixture Cause text, matching TaskID, every-frame controls, terminal distinctions and independent drain/Result barriers remain. No production/API, dependency allowlist or original command scope changed. The failed gate and prior worker evidence remain archived externally. This replacement source still requires its own complete G05 gate before freezing.
