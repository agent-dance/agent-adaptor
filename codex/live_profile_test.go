...
// the operator's native profile. Auth and explicit routing are validated by the
// shared test-only helper; only the API key is written into the fresh profile.
func alignmentLiveConfig(t *testing.T) Config {
	...
	root, profile, workspace := p.Home, p.Directory, p.Workspace
	...
	cfg := Config{CommonConfig: CommonConfig{Command: command, CWD: workspace, ExtraArgs: p.ExtraArgs, Env: []driver.EnvBinding{{Name: "CODEX_HOME", Value: profile}, {Name: "HOME", Value: root}, {Name: "USERPROFILE", Value: root}}}, Model: model}
...
func TestAlignmentLiveGateCanary(t *testing.T) {
	...
	t.Setenv(livetest.RouteEnv, filepath.Join(bin, "absent-route-file"))
