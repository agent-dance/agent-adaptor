// codex-sessions demonstrates the Thread half of the v1 API (design doc §2.4,
// scenario S3): four session modes collapse into four Thread actions on a
// host-owned key.
//
//	legacy WithSessionKey(ns, key)              → agent.Thread(key)
//	legacy WithNewSession(ns, key)              → agent.NewThread(key)
//	legacy WithContinueSession(sessionID)       → agent.Thread(key, adaptor.ResumeOnly())
//	legacy WithForkSession(id, ns, forkKey)     → th.Fork(forkKey)
//
// v1 keeps exactly two consumer-visible identity layers: the thread key (the
...
// Usage:
//
//	go run ./examples/codex-sessions -agent=codex -key=support/issue-42
package main
