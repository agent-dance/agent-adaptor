...
func main() {
	...
	key := flag.String("key", "codex-sessions", "Logical session key")
	forkKey := flag.String("fork-key", "codex-sessions-fork", "Logical session key for the forked branch")
	prompt := flag.String("prompt", "Reply with a short acknowledgement for the codex-sessions example.", "Prompt to send to Codex")
	...
	exampleutil.Must(err, "build codex-sessions example")

	first := runWithTimeout(sdk, *timeout, *prompt, agentadaptor.WithSessionKey(*namespace, *key))
	...
	second := runWithTimeout(sdk, *timeout, *prompt, agentadaptor.WithSessionKey(*namespace, *key))
	...
	continued := runWithTimeout(sdk, *timeout, *prompt, agentadaptor.WithContinueSession(first.Session.ID))
	...
	restarted := runWithTimeout(sdk, *timeout, *prompt, agentadaptor.WithNewSession(*namespace, *key))
	...
	forked := runWithTimeout(sdk, *timeout, *prompt, agentadaptor.WithForkSession(first.Session.ID, *namespace, *forkKey))
...
func runWithTimeout(sdk agentadaptor.SDK, timeout time.Duration, prompt string, opts ...agentadaptor.RunOption) agentadaptor.RunResult {
