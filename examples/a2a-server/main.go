// a2a-server starts an in-process A2A server around a real local Agent, then
...
// conversation of its own using a collision-free, namespace-encoded Thread
// key, so follow-up messages in the same context continue where they left off. Swap in
// a2a.Stateless() (the default) for one-shot task servers.
//
...
// Usage:
//
//	go run ./examples/a2a-server -agent=codex
//	go run ./examples/a2a-server -agent=claude -prompt="Reply with one sentence"
//	go run ./examples/a2a-server -serve-only -addr=127.0.0.1:8080
//
...
func main() {
	...
	contextID := flag.String("context", "a2a-demo/thread-1", "A2A contextId mapped to an Agent thread key")
	...
	ai := adaptor.New(
		...
		adaptor.WithMetadata("example", "a2a-server"),
	...
	server := bridgea2a.NewServer(ai, bridgea2a.ServerOptions{
		...
		// Each A2A contextID becomes a collision-free, namespace-encoded Thread key.
...
func newIsolation(agent, workspace, profile string, keep bool) (isolationConfig, error) {
	...
	if strings.TrimSpace(workspace) == "" || strings.TrimSpace(profile) == "" {
		root, err := os.MkdirTemp("", "agent-adaptor-a2a-server-*")
		if err != nil {
