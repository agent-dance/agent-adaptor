// inspect demonstrates read-only Driver probes and Agent inspection. Each
// Agent is an ordinary Go value with its own Inspector; hosts that need several
// agents keep several values or build their own application-level registry.
//
// Usage:
//
//	go run ./examples/inspect -agent=codex
package main
...
func main() {
	...
	writer := adaptor.New(
		...
		exampleutil.NonInteractive(writerCfg.Agent, adaptor.WorkspaceWrite),
	)
	...
	reviewer := adaptor.New(
		...
		exampleutil.NonInteractive(reviewCfg.Agent, adaptor.WorkspaceWrite),
	)
