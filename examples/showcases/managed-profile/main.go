...
func main() {
	...
	workspaceDir, err := os.MkdirTemp("", "agent-adaptor-managed-profile-*")
		...
		Hooks: []agentadaptor.HookSpec{
			{
				...
				Handler: agentadaptor.HookHandler{
					...
					Env:     map[string]string{"AUDIT_STREAM": "managed-profile"},
				},
	...
	sdk := agentadaptor.New(
		agentadaptor.WithDefaultAgent(exampleutil.NewLiveAgentBinding(
			...
			agentadaptor.WithDefaultMetadata("example", "managed-profile"),
		)),
...
func checkRunSucceeded(label string, result agentadaptor.RunResult) {
	if result.Failure != nil {
		exampleutil.Fatalf("%s run failed: %s (%s)", label, result.Failure.Message, result.Failure.Code)
	}
...
func defaultConfigPatches(cfg exampleutil.LiveAgentConfig) []agentadaptor.ProfileConfigPatch {
	...
	case exampleutil.AgentClaude:
		return []agentadaptor.ProfileConfigPatch{
			...
			{Key: "example-env", Capability: "env", Values: map[string]any{"AGENT_ADAPTOR_EXAMPLE": "managed-profile"}},
		}
