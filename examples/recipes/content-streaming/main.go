// content-streaming is a minimal example showing how to consume
// RunHandle.StreamEvents() in pure Go to build a character-level chat UI.
//
// Usage:
//
//	go run ./examples/recipes/content-streaming -agent=claude -prompt="Write a haiku about streaming"
//
...
func main() {
	...
	environment, err := exampleutil.NewTemporaryAgentEnvironment("content-streaming")
	...
	handle, err := sdk.Start(ctx, *prompt,
		...
		agentadaptor.WithSessionKey("examples", "content-streaming"),
	...
	if result.Failure != nil {
		fmt.Fprintln(os.Stderr, "run failed:", result.Failure.Message)
		os.Exit(1)
	}
