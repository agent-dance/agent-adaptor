...
func main() {
	...
	environment, err := exampleutil.NewTemporaryAgentEnvironment("async-events")
	...
	handle, err := sdk.Start(ctx, *prompt, exampleutil.NonInteractiveRunOption(agentadaptor.IsolationReadOnly))
	exampleutil.Must(err, "start async-events example")
	...
	exampleutil.Check(result.Failure == nil, "async run failed: %#v", result.Failure)
