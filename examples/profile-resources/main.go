// profile-resources shows the second half of the profile vocabulary: not only
...
func main() {
	...
	ai := adaptor.New(
		...
		adaptor.WithMetadata("example", "profile-resources"),
...
func mustRun(ai *adaptor.Agent, ctx context.Context, prompt string, opts ...adaptor.CallOption) *adaptor.Result {
	...
	if err != nil {
		...
		exampleutil.Fatalf("run profile-resources example: %v", err)
	}
	return res
}
