// profiles materializes a complete provider profile — skills, MCP,
...
//
// Profile mutation stays on the Agent because it changes what a run does;
// Inspect() exposes only read-only panels.
//
// Usage:
//
//	go run ./examples/profiles -agent=codex
//	go run ./examples/profiles -agent=claude -profile-mode=native -run
//
...
func main() {
	...
	resolvedWorkspace := ensureDir("workspace", *workspace, "agent-adaptor-profiles-workspace-*")
	...
	mcpServerPackage := "./examples/profiles/mcpserver"
	hookPackage := "./examples/profiles/hook"
	...
	ai := adaptor.New(
		...
		exampleutil.NonInteractive(agentCfg.Agent, adaptor.Unrestricted),
	)
...
func selectProfileOption(agent string, layout providerLayout, mode, requested string) (adaptor.Option, string, string, string) {
	...
	case "native":
		...
		artifacts := ensureDir("artifact", "", "agent-adaptor-profiles-artifacts-*")
...
func chooseProfileTarget(agent, requested string) string {
	...
	for attempt := 0; attempt < 100; attempt++ {
		candidate := filepath.Join(os.TempDir(), fmt.Sprintf("agent-adaptor-%s-profiles-%d", agent, time.Now().UnixNano()))
