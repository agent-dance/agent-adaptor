...
type DefaultMaterializerOption func(*materializerConfig)
...
// It is also what Build() installs when the host does not supply a
// materializer of its own; the public vocabulary packages re-export it
// so hosts can compose a chain-of-responsibility around it.
...
// archiveExtraction is the result of safely walking an archive.
//
...
func extractArchive(raw []byte, format SkillArchiveFormat, subpath string, cfg materializerConfig) (archiveExtraction, error) {
	if format == SkillArchiveAuto {
		format = sniffArchiveFormat(raw)
	}
	switch format {
	case SkillArchiveZip:
		return extractZip(raw, subpath, cfg)
	case SkillArchiveTar:
		return extractTar(raw, subpath, cfg)
	case SkillArchiveTarGz:
		return extractTarGz(raw, subpath, cfg)
	default:
		return archiveExtraction{}, fmt.Errorf("%w: unable to identify archive format", errArchiveFormat)
	}
}
...
func extractZip(raw []byte, subpath string, cfg materializerConfig) (archiveExtraction, error) {
	if len(raw) == 0 {
		return archiveExtraction{}, fmt.Errorf("%w: empty archive", errArchiveFormat)
	}
	zr, err := zip.NewReader(bytes.NewReader(raw), int64(len(raw)))
	if err != nil {
		return archiveExtraction{}, fmt.Errorf("%w: zip open: %v", errArchiveFormat, err)
	}
	if len(zr.File) > cfg.maxArchiveEntries {
		return archiveExtraction{}, fmt.Errorf("%w: archive has %d entries, limit %d", errArchiveFormat, len(zr.File), cfg.maxArchiveEntries)
	}
	...
	for _, entry := range zr.File {
		...
		if errMsg != "" {
			return archiveExtraction{}, fmt.Errorf("%w: %s", errArchiveFormat, errMsg)
		}
		if !ok {
			continue
		}
		if int64(entry.UncompressedSize64) > cfg.maxFileBytes {
			return archiveExtraction{}, fmt.Errorf("%w: entry %q is %d bytes, limit %d", errArchiveFormat, entry.Name, entry.UncompressedSize64, cfg.maxFileBytes)
		}
		rc, err := entry.Open()
		if err != nil {
			return archiveExtraction{}, fmt.Errorf("%w: zip entry %q: %v", errArchiveFormat, entry.Name, err)
		}
		...
		_ = rc.Close()
		if readErr != nil {
			return archiveExtraction{}, fmt.Errorf("%w: zip entry %q read: %v", errArchiveFormat, entry.Name, readErr)
		}
		if int64(len(buf)) > cfg.maxFileBytes {
			return archiveExtraction{}, fmt.Errorf("%w: entry %q exceeds per-file limit %d", errArchiveFormat, entry.Name, cfg.maxFileBytes)
		}
		files[rel] = buf
	}
	return archiveExtraction{Files: files}, nil
}

func extractTar(raw []byte, subpath string, cfg materializerConfig) (archiveExtraction, error) {
	return extractTarReader(bytes.NewReader(raw), subpath, cfg)
}

func extractTarGz(raw []byte, subpath string, cfg materializerConfig) (archiveExtraction, error) {
	gz, err := gzip.NewReader(bytes.NewReader(raw))
	if err != nil {
		return archiveExtraction{}, fmt.Errorf("%w: gzip open: %v", errArchiveFormat, err)
	}
	defer func() { _ = gz.Close() }()
	return extractTarReader(gz, subpath, cfg)
}

func extractTarReader(r io.Reader, subpath string, cfg materializerConfig) (archiveExtraction, error) {
	tr := tar.NewReader(r)
	sub := normalizeSubpath(subpath)
	files := map[string][]byte{}
	entries := 0
	for {
		...
		if err != nil {
			return archiveExtraction{}, fmt.Errorf("%w: tar header: %v", errArchiveFormat, err)
		}
		entries++
		if entries > cfg.maxArchiveEntries {
			return archiveExtraction{}, fmt.Errorf("%w: archive has more than %d entries", errArchiveFormat, cfg.maxArchiveEntries)
		}
		...
		if hdr.Size > cfg.maxFileBytes {
			return archiveExtraction{}, fmt.Errorf("%w: entry %q is %d bytes, limit %d", errArchiveFormat, hdr.Name, hdr.Size, cfg.maxFileBytes)
		}
		buf, readErr := io.ReadAll(io.LimitReader(tr, cfg.maxFileBytes+1))
		if readErr != nil {
			return archiveExtraction{}, fmt.Errorf("%w: tar entry %q read: %v", errArchiveFormat, hdr.Name, readErr)
		}
		if int64(len(buf)) > cfg.maxFileBytes {
			return archiveExtraction{}, fmt.Errorf("%w: entry %q exceeds per-file limit %d", errArchiveFormat, hdr.Name, cfg.maxFileBytes)
		}
		files[rel] = buf
	}
	return archiveExtraction{Files: files}, nil
