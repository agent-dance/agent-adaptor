...
func TestDriverInjectsCapturedConfigIntoRunRequest(t *testing.T) {
	...
	if !ok {
		t.Fatalf("req.Config is %T, want cursor.Config", req.Config)
	}
	...
	override := Config{Model: "claude-sonnet-4"}
...
func TestConfigMapsEveryCommonFieldToEngine(t *testing.T) {
	...
	common := CommonConfig{
		Command: "cursor-bin", CWD: "/repo", Env: []driver.EnvBinding{{Name: "KEY", Value: "value"}}, Instructions: instructions,
...
func TestDriverCapturedConfigAndEngineConversionAreMutationIsolated(t *testing.T) {
	...
	}, Model: "original-model", Mode: "agent"}
	...
	cfg.Model, cfg.Mode = "mutated-model", "ask"
...
func assertConfigSnapshot(t *testing.T, got Config) {
	t.Helper()
	if got.Model != "original-model" || got.Mode != "agent" || got.Env[0].Value != "original" || got.ExtraArgs[0] != "original" || got.Instructions.ID != "original" || got.Instructions.Native["nested"].(map[string]any)["value"] != "original" || got.WorkspaceStrategy.BaseRef != "original" || got.WorkspaceRuntime.Services[0].ID != "original" || got.WorkspaceRuntime.Services[0].Metadata["key"] != "original" {
		t.Fatalf("captured Config was mutated through caller-owned data: %#v", got)
	}
}

func assertEngineCommonConfigSnapshot(t *testing.T, got engine.CursorConfig) {
	t.Helper()
	if got.Model != "original-model" || got.Mode != "agent" || got.Env[0].Value != "original" || got.ExtraArgs[0] != "original" || got.Instructions.ID != "original" || got.Instructions.Native["nested"].(map[string]any)["value"] != "original" || got.WorkspaceStrategy.BaseRef != "original" || got.WorkspaceRuntime.Services[0].ID != "original" || got.WorkspaceRuntime.Services[0].Metadata["key"] != "original" {
...
func TestDriverPreservesOptionalCapabilityInterfaces(t *testing.T) {
	...
	if _, ok := d.(driver.EnvironmentProbe); !ok {
		t.Error("driver.EnvironmentProbe lost on configured driver")
	}
	if _, ok := d.(driver.ModelLister); !ok {
		t.Error("driver.ModelLister lost on configured driver")
	}
	if _, ok := d.(driver.ModelDetector); !ok {
		t.Error("driver.ModelDetector lost on configured driver")
	}
	if _, ok := d.(driver.ProfileReporter); !ok {
		t.Error("driver.ProfileReporter lost on configured driver")
	}
	if _, ok := d.(driver.ConfigSchemaProvider); !ok {
		t.Error("driver.ConfigSchemaProvider lost on configured driver")
	}
	if _, ok := d.(driver.QuotaProbe); !ok {
		t.Error("driver.QuotaProbe lost on configured driver")
	}
	if _, ok := d.(driver.SkillSupport); !ok {
		t.Error("driver.SkillSupport lost on configured driver")
	}
	if _, ok := d.(driver.SessionCodecProvider); !ok {
		t.Error("driver.SessionCodecProvider lost on configured driver")
	}
	if _, ok := d.(engine.ProfileResourceDriver); !ok {
		t.Error("profile resource capability lost on configured driver")
	}
}
