...
func TestInteractiveDecisionErrorAbortsDriverUnchanged(t *testing.T) {
	t.Setenv("CLAUDE_CONFIG_DIR", "")
	home := t.TempDir()
	control := `{"type":"control_request","request_id":"req-abort","request":{"subtype":"can_use_tool","tool_name":"Bash","tool_use_id":"tool-abort","input":{"command":"pwd"}}}`
	terminal := `{"type":"result","subtype":"success","is_error":false,"session_id":"session-after-abort","result":"must not override approval error"}`
	command := testutil.WriteCommand(t, home, "fake-claude-decision-abort",
		"#!/bin/sh\nprintf '%s\\n' '"+control+"'\ncat >/dev/null\nprintf '%s\\n' '"+terminal+"'\n",
		"@echo off\r\necho "+control+"\r\nmore > nul\r\necho "+terminal+"\r\n",
	)
	wantErr := errors.New("approval callback failed")
...
// TestInteractive_PlanApproved_InjectsApproveToolResult drives the full
...
func TestInteractive_PlanApproved_InjectsApproveToolResult(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-plan.jsonl")
	...
	if req.ToolCallID != "toolu_plan_interactive" {
...
// TestInteractive_PlanRejected_SetsFailureAndDenies covers OnReject=FailureAbort:
...
func TestInteractive_PlanRejected_SetsFailureAndDenies(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-plan.jsonl")
...
// TestInteractive_QuestionAnswered_MultipleChoice verifies the multipleChoice
...
func TestInteractive_QuestionAnswered_MultipleChoice(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-question.jsonl")
...
// TestInteractive_QuestionAnswered_FreeText verifies the freeText path: there
...
func TestInteractive_QuestionAnswered_FreeText(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-question-freetext.jsonl")
...
func TestExtractInteractivePayload_Shapes(t *testing.T) {
	...
	t.Run("question prompt fallback", func(t *testing.T) {
		prompt, _, _ := extractInteractivePayload(
			agentadaptor.HumanDecisionQuestion,
...
// TestInteractive_NonWhitelistedTool_AutoApprove_WritesControlResponse verifies
...
func TestInteractive_NonWhitelistedTool_AutoApprove_WritesControlResponse(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-bash.jsonl")

	sink := newFakeInteractiveSink(func(agentadaptor.DecisionRequest) (agentadaptor.DecisionResponse, error) {
		t.Fatal("RequestDecision must not be called for non-whitelisted tools in interactive mode")
	...
	cr := decodeControlResponseFrame(t, frames[0])
	if cr.RequestID != "req-bash-interactive" {
		t.Errorf("request_id: %q", cr.RequestID)
	}
	if cr.Behavior != "allow" {
		t.Fatalf("Bash auto-approve must produce allow, got %q", cr.Behavior)
	}
	if cr.ToolUseID != "toolu_bash_interactive" {
		t.Errorf("toolUseID: %q", cr.ToolUseID)
	}
...
// TestInteractive_IsReplayFrame_Ignored covers the ack stream: the CLI echoes
...
func TestInteractive_IsReplayFrame_Ignored(t *testing.T) {
...
func TestInteractive_NonWhitelistedTool_UnsetPermission_UsesDecisionSink(t *testing.T) {
	data := mustReadFixture(t, "streaming-interactive-bash.jsonl")
	sink := newFakeInteractiveSink(func(req agentadaptor.DecisionRequest) (agentadaptor.DecisionResponse, error) {
		if req.Kind != agentadaptor.HumanDecisionPermission || req.Source != "claude.bash" {
			t.Fatalf("ordinary tool request = %+v, want Permission/Bash", req)
		}
		return agentadaptor.DecisionResponse{RequestID: req.RequestID, Result: agentadaptor.DecisionApproved}, nil
	})
	stdin := &fakeStdin{}
	p := newClaudeParser(sink)
	// Enter interactive mode for plan review while leaving Permission unset.
	// Its effective value is Ask, so an ordinary tool must still receive one
	// answerable decision request and one control response.
	p.setHITLContext("run-unset-permission", agentadaptor.HumanDecisionPolicy{PlanReview: agentadaptor.HumanDecisionAsk})
	p.enableStreaming("run-unset-permission")
	p.enableInteractive(context.Background(), sink, stdin)
	if err := p.onChunk("stdout", data, time.Now().UTC()); err != nil {
		t.Fatal(err)
	}
	if got := len(sink.requests); got != 1 {
		t.Fatalf("Decision requests = %d, want 1", got)
	}
	frames := stdin.snapshot()
	if len(frames) != 1 {
		t.Fatalf("control responses = %d, want 1", len(frames))
	}
	if got := decodeControlResponseFrame(t, frames[0]).Behavior; got != "allow" {
		t.Fatalf("control behavior = %q, want allow", got)
	}
}

// TestWantsInteractiveClaude covers the mode-selection helper. The helper
// inspects raw policy fields: a zero-valued policy stays in observational mode
...
func TestWantsInteractiveClaude(t *testing.T) {
	cases := []struct {
		...
		{"zero policy stays observational", agentadaptor.HumanDecisionPolicy{}, false},
		{"explicit permission ask needs control", agentadaptor.HumanDecisionPolicy{
			Permission: agentadaptor.HumanDecisionAsk,
		}, true},
		...
		{"plan auto-reject needs control", agentadaptor.HumanDecisionPolicy{
			Permission: agentadaptor.HumanDecisionAutoApprove,
			PlanReview: agentadaptor.HumanDecisionAutoReject,
		}, true},
		{"permission auto-reject needs control", agentadaptor.HumanDecisionPolicy{
