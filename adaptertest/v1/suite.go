...
func TestDriver(t *testing.T, newDriver func() driver.Driver, opts ...Option) {
	...
	t.Run("capability_declarations", func(t *testing.T) { checkDeclarations(t, d, desc) })
...
func checkDeclarations(t *testing.T, d driver.Driver, desc driver.Descriptor) {
	t.Helper()
	// CAP-01: resume declaration implies a session codec.
	if desc.Sessions.SupportsResume {
		provider, ok := d.(driver.SessionCodecProvider)
		if !ok {
			t.Error("CAP-01: Sessions.SupportsResume=true but the driver does not implement driver.SessionCodecProvider (SessionCodecProvider docs: resume-capable drivers expose a stable session parameter contract)")
...
func checkSessionCodec(t *testing.T, d driver.Driver, desc driver.Descriptor, c *suiteConfig) {
	...
	if codec == nil {
		t.Fatal("CAP-01: SessionCodec() returned nil")
	}
	...
	if again := codec.Name(); again != name {
