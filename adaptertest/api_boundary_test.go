...
// These tests freeze rejected parallel API surfaces. They intentionally use
...
func TestNoParallelSubagentBusAPI(t *testing.T) {
	root := moduleRoot(t)
	handler := filepath.Join(root, "bridges", "sse", "handler.go")
	optionsName := "Options"
	file := parseGoFile(t, handler)
	if !hasType(file, optionsName) {
		t.Fatal("cannot locate the SSE Options declaration")
	}
	fields := exportedStructFields(t, file, optionsName)
	if hasAnonymousStructField(t, file, optionsName) {
		t.Fatalf("%s.%s has an anonymous field; a promoted delegation overlay can bypass the explicit SubagentBus guard", filepath.Base(handler), optionsName)
	}
	if contains(fields, "SubagentBus") {
		t.Fatalf("%s.%s reintroduced SubagentBus; team.Option must inject SubagentUpdate into the Runner's single Event stream", filepath.Base(handler), optionsName)
	}
	for _, imp := range file.Imports {
		path := strings.Trim(imp.Path.Value, `"`)
		if strings.Contains(path, "a2adelegation") || strings.Contains(path, "subagentstream") {
			t.Fatalf("SSE handler imports %q; it must consume the Runner Event stream without a delegation overlay", path)
		}
	}
}

func TestPresentationPolicyStaysOutsideCore(t *testing.T) {
	...
	if methods := exportedReceiverMethods(files, "SubagentEventKind"); len(methods) != 0 {
		t.Fatalf("SubagentEventKind gained core presentation methods %v; hosts render string labels", methods)
	}
	if methods := exportedReceiverMethods(files, "ToolCall"); len(methods) != 0 {
		t.Fatalf("ToolCall gained core presentation methods %v; argument preview requires host-specific truncation and redaction", methods)
	}
		...
		for _, decl := range file.Decls {
			...
			if fn.Recv != nil && receiverName(fn.Recv.List[0].Type) == "Result" {
				if returnsNamedType(fn.Type.Results, "string") {
					t.Fatalf("Result method %s returns a presentation string; callers already have independent Text and Summary fields", fn.Name.Name)
				}
				if fieldListContainsNamedType(fn.Type.Results, "RunAttachment") {
					t.Fatalf("Result method %s exposes RunAttachment; Services is the only runtime observation accessor", fn.Name.Name)
				}
				continue
			}
			if fn.Recv != nil {
			...
			if acceptsNamedType(fn.Type.Params, "ToolCall") && returnsNamedType(fn.Type.Results, "string") {
				t.Fatalf("exported helper %s accepts ToolCall and returns string; Args presentation belongs to the host", fn.Name.Name)
			}
			if acceptsNamedType(fn.Type.Params, "Result") && returnsNamedType(fn.Type.Results, "string") {
				t.Fatalf("exported helper %s accepts Result and returns string; Summary/Text fallback belongs to the host", fn.Name.Name)
			}
			if acceptsNamedType(fn.Type.Params, "Result") && fieldListContainsNamedType(fn.Type.Results, "RunAttachment") {
				t.Fatalf("exported helper %s exposes RunAttachment from Result; Services is the only runtime observation accessor", fn.Name.Name)
			}
		}
	}
	...
	for _, field := range exportedStructFields(t, result, "Result") {
		if strings.Contains(strings.ToLower(field), "attachment") {
			t.Fatalf("Result reintroduced attachment field %q; use Services for observed runtime reports", field)
		}
	}
}

func TestNoProviderStreamingCapabilityQuery(t *testing.T) {
	...
	for _, file := range files {
		for _, decl := range file.Decls {
			...
			if ok && fn.Recv == nil && fn.Name.IsExported() && strings.Contains(strings.ToLower(fn.Name.Name), "stream") {
				t.Fatalf("exported function %s creates a parallel provider-streaming surface", fn.Name.Name)
			}
		}
	}
	for _, receiver := range []string{"Agent", "Thread", "Inspector"} {
		for _, method := range exportedReceiverMethods(files, receiver) {
			...
			if method != "Stream" && strings.Contains(lower, "stream") {
				t.Fatalf("%s.%s is a provider-streaming query; Runner.Stream availability and provider transport fidelity are separate contracts", receiver, method)
			}
		}
	}
}
	...
	for _, file := range files {
		for _, decl := range file.Decls {
			...
			if mapped["Text"] != "Output" || mapped["Summary"] != "Summary" {
				t.Fatalf("resultFromResponse mappings are Text<-resp.%s Summary<-resp.%s; both layers must remain direct and independent", mapped["Text"], mapped["Summary"])
			}
			return
		}
	}
	t.Fatal("resultFromResponse not found")
}
