...
//   - empty: SDK computes sha256(archive bytes) at materialize time and
//     uses it as the cache key. Hosts that already know the archive's
//     digest should pass it as Fingerprint to skip the rehash.
//
// Hosts whose store identifies content by an opaque ID (store-id +
// version) can pass that string as Fingerprint instead of a sha256.
// The materializer treats Fingerprint as an opaque cache key and does
...
// checking; pass Fingerprint on the SkillFromArchive (or wrap this
// Archive in your own closure) for that.
//
